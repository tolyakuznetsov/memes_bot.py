...
class TelegramBot:
    ...
    def setup_handlers(self):
        self.dp.register_message_handler(
            self.end_command_handler, commands=['end'], state='*'
        )
        self.dp.register_message_handler(
            self.player_count_handler, state=states.UsersStates.wait_response
        )
        self.dp.register_message_handler(
            self.start_command_handler, commands=['start']
        )
        self.dp.register_callback_query_handler(
            self.start_game_handler, lambda c: c.data == 'button_start_game'
        )
        self.dp.register_callback_query_handler(
            self.rules_handler, lambda c: c.data == 'button_rules'
        )
        self.dp.register_callback_query_handler(
            self.start_turn, lambda c: c.data == 'start_turn'
        )
        self.dp.register_callback_query_handler(
            self.send_image_to_chat,
            lambda query: query.data.startswith('image_path')
        )
        self.dp.register_callback_query_handler(
            self.send_description,
            lambda c: c.data == 'button_description'
        )
        self.dp.register_callback_query_handler(
            self.user_pick_hero,
            lambda c: c.data.startswith('hero_')
        )
        self.dp.register_callback_query_handler(
            self.stop_pick_hero,
            lambda c: c.data == 'button_stop_pick_hero'
        )
    ...
    async def start_command_handler(message: types.Message):
        ...
        if chat_id == 274921311:
            await message.reply(
                'Привет! Я бот для игры в ..., добавь меня в чат, чтобы начать игру',
    ...
    async def user_pick_hero(self, callback_query: types.CallbackQuery):
        ...
        if users_hero:
            ...
            db.db_insert_user_hero(chat_id, user_id, hero)  # Записываем в БД героя,
            # которого выбрал юзер.

            last_keyboard = db.db_select_keyboard(_keyboard, chat_id)  # Получаем последнюю
            # сохраненную клавиатуру
            last_keyboard['inline_keyboard'] = [
                x for x in last_keyboard["inline_keyboard"]
                if not any(y["callback_data"] == button_data for y in x)
            ]
            if len(last_keyboard['inline_keyboard']) == 1:
                ...
                if ((len(last_keyboard['inline_keyboard']) == 1) and
                        (last_keyboard['inline_keyboard'][0][0]['callback_data']
    ...
    async def stop_pick_hero(self, callback_query: types.CallbackQuery):
        ...
        if count_users == 0:
            ...
            await self.bot.send_message(
                chat_id,
                text='Выбран только 1 персонаж, ты не сможешь играть 1'
            )
        else:
            db.db_delete_keyboard(_keyboard, chat_id)
            await self.bot.delete_message(chat_id=chat_id, message_id=message_id)
        ...
        for user in user_list:
            ...
            if image_list:
                for image_bytes, image_path in image_list:
                    ...
                    file_id = message.photo[-1].file_id  # получаем file_id.
                    # картинки на сервере телеграмма.
            ...
            chat_link = await self.bot.export_chat_invite_link(
                chat_id=callback_query.message.chat.id
            )
            buttons = kb.Buttons()
            ...
            await self.bot.send_message(
                ...
                text='Нажми кнопку, чтобы вернуться в чат:',
                reply_markup=inline_kb4)

            await self.bot.send_message(
                callback_query.from_user.id,
                text='Все картинки уже были отправлены :('
            )
            await self.bot.send_message(
                callback_query.from_user.id,
                text='Нажми кнопку, чтобы вернуться в чат:',
    ...
    async def send_image_to_chat(self, callback_query: types.CallbackQuery):
        chat_id = db.get_mapp_user_chat(callback_query.from_user.id)
        type_of_state = 'turn'
        state = int(db.db_get_state(chat_id, type_of_state))
        if not state:
            await self.bot.send_message(
                callback_query.from_user.id,
                text='Ты не можешь отправлять картинки вне хода'
            )
        elif state:
            ...
            if was_sent_img:
                await self.bot.send_message(
                    callback_query.from_user.id,
                    text='Эта карта уже была отправлена'
                )
            elif was_sent_imgs:
                await self.bot.send_message(
                    callback_query.from_user.id,
                    text='Ты уже отправлял карты в этот ход'
                )
            else:
                with requests.get(image_url, stream=True) as r:
                    r.raise_for_status()
                    with io.BytesIO(r.content) as image:
                        caption = 'Карточка от ' + callback_query.from_user.full_name
    ...
    async def start_turn(self, callback_query: types.CallbackQuery):
        ...
        message_30_sec = await self.bot.send_message(
            chat_id,
            text='У вас 30 секунд на отправку карт в чат!'
        )
